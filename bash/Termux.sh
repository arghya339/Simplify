...
locale=$(getprop persist.sys.locale)  # Get System Locale
[ -z $locale ] && locale=$(getprop ro.product.locale)  # Get Locale
...
dependencies() {
  ...
  pkgInstall "html2text"  # html2text install/update
...
installTermuxAPI() {
  if ! am start -n "com.termux.api/com.termux.shared.activities.ReportActivity" &>/dev/null; then
    ...
    [ -f $Download/termux-api-app_${tag_name}+github.debug.apk ] && apkInstall "$Download/termux-api-app_${tag_name}+github.debug.apk" "com.termux.api/com.termux.shared.activities.ReportActivity"
  else
...
if [ $AutoUpdatesTermux == true ]; then
  ...
  if [ "$TERMUX_VERSION" != "$(sed 's/^v//' <<< "$tag_name")" ]; then
    ...
    if [ $su == true ]; then
      ...
      [ "$(su -c 'getenforce 2>/dev/null')" = "Enforcing" ] && { su -c "setenforce 0"; touch "$simplifyNext/writeSELinux"; }
    ...
    if [ -f "$filePath" ]; then
      if [ $su == true ]; then
        if [ "$(su -c 'getenforce 2>/dev/null')" = "Permissive" ] && [ -f "$simplifyNext/writeSELinux" ]; then
          su -c "setenforce 1"  # set SELinux to Enforcing mode to block unauthorized operations
          rm -f "$simplifyNext/writeSELinux"
        fi
