...
dlPatchedApps() {
  ...
  selected_pre_patched_app=0
  while true; do
    menu names bButtons "" "" "$selected_pre_patched_app" && selected_pre_patched_app=$selected || break
    name="${names[selected_pre_patched_app]}"
    channels=($(jq -r --arg n "$name" '.[] | select(.name==$n) | .assets | to_entries[] | select(.value | length > 0) | .key' $simplifyNext/dl.json))
    selected_channel=0
    if menu channels bButtons "" "" "$selected_channel"; then
      ...
      if [ ${#archs[@]} -gt 1 ]; then
        for ((i=0; i<${#archs[@]}; i++)); do [ "${archs[i]}" == "$cpuAbi" ] && { selected_arch=$i; archs[i]="${archs[i]} (Recommended)"; }; done
      else
        selected_arch=0
      fi
      if menu archs bButtons "" "" "$selected_arch"; then
        selected_arch=$selected
        #arch="${archs[selected_arch]}"
        #assets_name=$(jq -r --arg n "$name" --arg c "$channel" --arg a "$arch" '.[] | select(.name==$n) | .assets[$c][$a]' $simplifyNext/dl.json)
        assets_name="${assets_names[selected_arch]}"
            ...
            if menu appsOptions bButtons; then
              ...
              case "$appsOption" in
                ...
                Share)
                  ...
                  ;;
                Open) shellCmd "monkey -p $package -c android.intent.category.LAUNCHER 1" &>/dev/null ;;
                Install|Reinstall)
                  installed=true
                ...
                Uninstall)
                  ...
                  if [ $isAndroid == false ]; then
                    ...
                    if [ $su == true ] || rish -c "id" &>/dev/null || adb -s $(adb devices 2>/dev/null | grep "device$" | awk '{print $1}' | tail -1) shell "id" &>/dev/null; then
                      ...
                      am start -a android.intent.action.UNINSTALL_PACKAGE -d package:"$package" &>/dev/null
                      sleep 6
