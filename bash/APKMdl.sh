...
cf_chl_error() {
  ...
  if [ $isAndroid == true ]; then
    ...
    [ -d "/Applications/Cloudflare WARP.app" ] && open -a "Cloudflare WARP" || { pkgInstall "cloudflare-warp"; open -a "Cloudflare WARP"; }
  else
    #gtk-launch $(xdg-settings get default-web-browser) &>/dev/null
    [ -f /usr/share/applications/com.cloudflare.WarpTaskbar.desktop ] && warp-cli connect || xdg-open "https://pkg.cloudflareclient.com/" &>/dev/null
  fi
...
breadcrumbsMenu() {
  ...
  if ! grep -q "_cf_chl_" <<< "$appPageHtml"; then
    ...
    if [ "$hasBreadcrumbsMenu" == "true" ]; then
      ...
      mapfile -t htmlNames < <(jq -r '.[].name' <<< "$breadcrumbsMenuJson")
      names=(); for ((i=0; i<${#htmlNames[@]}; i++)); do names+=("$(html2text <<< "${htmlNames[i]}")"); done
...
fetchVariant() {
  ...
  if ! grep -q "_cf_chl_" <<< "$variantHtml"; then
    ...
    mapfile -t variantsTableRow < <(jq -r '.[] | select((.children | type) == "array" and (.children | length) == 4)
  | select(
  ...
  if [ -n "$cpuAbi" ]; then
    ...
    if [ $RipLib == true ]; then
      if [ "$fileExt" == "apks" ]; then
        ...
        [ $byBundletool == true ] && includeFile=("splits/base-master.apk" "splits/base-${cpuAbi//-/_}.apk" "splits/base-${lang}.apk" "splits/base-${lcd_dpi}.apk") || includeFile=("base.apk" "split_config.${cpuAbi//-/_}.apk" "split_config.${lang}.apk" "split_config.${lcd_dpi}.apk")
      elif [ "$fileExt" == "xapk" ]; then
        includeFile=("$pkgName.apk" "config.${cpuAbi//-/_}.apk" "config.${lang}.apk" "config.${lcd_dpi}.apk")
      elif [ "$fileExt" == "apkm" ]; then
        includeFile=("base.apk" "split_config.${cpuAbi//-/_}.apk" "split_config.${lang}.apk" "split_config.${lcd_dpi}.apk")
      fi
    elif [ $RipLib == false ]; then
      if [ "$fileExt" == "apks" ]; then
        [ $byBundletool == true ] && includeFile=("splits/base-master.apk" "splits/base-arm64_v8a.apk" "splits/base-armeabi_v7a.apk" "splits/base-x86_64.apk" "splits/base-x86.apk" "splits/base-${lang}.apk" "splits/base-${lcd_dpi}.apk") || includeFile=("base.apk" "split_config.arm64_v8a.apk" "split_config.arm64_v7a.apk" "split_config.x86_64.apk" "split_config.x86.apk" "split_config.${lang}.apk" "split_config.${lcd_dpi}.apk")
      elif [ "$fileExt" == "xapk" ]; then
        includeFile=("$pkgName.apk" "config.arm64_v8a.apk" "config.armeabi_v7a.apk" "config.x86_64.apk" "config.x86.apk" "config.${lang}.apk" "config.${lcd_dpi}.apk")
      elif [ $fileExt == "apkm" ]; then
        includeFile=("base.apk" "split_config.arm64_v8a.apk" "split_config.armeabi_v7a.apk" "split_config.x86_64.apk" "split_config.x86.apk" "split_config.${lang}.apk" "split_config.${lcd_dpi}.apk")
      fi
    fi
