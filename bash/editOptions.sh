  ...
  while true; do
    if [ ${#patchNamesContainOptions[@]} -gt 0 ]; then
      if menu patchNamesContainOptions cButtons patchDescriptionsContainOptions; then
        ...
        if [ "$ShowUniversalPatches" == true ]; then
          patchNameOptionsKeys=($(jq -r --arg pkg "$package" --arg pn "$patchNamecontainOptions" '.[] | select(.name == $pn) | select((.compatiblePackages == null) or any(.compatiblePackages[]?; if .name? and (.name|type)=="string" then .name == $pkg else .versions | if type=="object" then .packageName == $pkg elif type=="array" then any(.[].packageName? == $pkg) else false end end)) | .options[] | (if .key then .key else .name end)' $sourceDir/patches-$patchesVersion.json))
        else
          patchNameOptionsKeys=($(jq -r --arg pkg "$package" --arg pn "$patchNamecontainOptions" '.[] | select(.name == $pn) | select(any(.compatiblePackages[]?; if .name? and (.name|type)=="string" then .name == $pkg else .versions | if type=="object" then .packageName == $pkg elif type=="array" then any(.[].packageName? == $pkg) else false end end)) | .options[] | (if .key then .key else .name end)' $sourceDir/patches-$patchesVersion.json))
        fi
        if [ "$ShowUniversalPatches" == true ]; then
