...
dependencies() {
  ...
  pkgInstall "bash"  # install
  pkgInstall "busybox"  # upgrade
  #pkgInstall "grep"  # part of busybox: apk info --who-owns $(which grep)
  #pkgInstall "gawk"  # gnu awk installation not required, becouse awk comes with busybox
  #pkgInstall "sed"  # comes with busybox
  #pkgInstall "findutils" # gnu findutils not needed due to busybox provide find: apk info --who-owns $(which find)
  pkgInstall "curl"  # install
  pkgInstall "aria2"  # install
  pkgInstall "jq"  # install
  pkgInstall "html2text"  # install
  pkgInstall "pup"  # install
  pkgInstall "libarchive-tools"  # install
  pkgInstall "pv"  # install
  ...
  pkgInstall "glow"  # install
  pkgInstall "$jdk"  # install
  pkgInstall "android-tools"  # (adb, fastboot)
