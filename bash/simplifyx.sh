...
EOF
      ...
      sourceOptions=(viewChangelog patchApp usePrereleases autoUpdates manuallyUpdating viewPatches displayName sourceCode deleteSource)
      [ "$microg" != "null" ] && sourceOptions+=(installMicroG)
      [ $cliv -ge 5 ] && sourceOptions+=(importAssets)
      ...
      while true; do
        if menu sourceOptions bButtons; then
          ...
          case "$sourceOption" in
            viewChangelog) glow $sourceDir/CHANGELOG.md ;;
            patchApp)
            ...
            usePrereleases)
              prereleases=$(jq -r --arg source "$source" '.[] | select(.source == $source) | .prereleases' $simplifyNext/sources.json)
              pButtons=("<true>" "<false>"); confirmPrompt "usePrereleases" "pButtons" "$prereleases" && prereleases=true || prereleases=false
            ...
            autoUpdates)
              autoupdates=$(jq -r --arg source "$source" '.[] | select(.source == $source) | .autoupdates' $simplifyNext/sources.json)
              pButtons=("<true>" "<false>"); confirmPrompt "autoUpdates" "pButtons" "$autoupdates" && autoupdates=true || autoupdates=false
            ...
            importAssets)
              ...
              ;;
            displayName)
              read -r -p "displayName: " -i "$source" -e displayName
              displayName=$(sed 's/ /-/g' <<< "$displayName")
              [ -n "$displayName" ] && jq --arg source "$source" --arg name "$displayName" 'map(if .source == $source then .source = $name else . end)' $simplifyNext/sources.json > tmp.json && mv tmp.json $simplifyNext/sources.json
              break
              ;;
            sourceCode)
