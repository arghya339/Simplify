...
isAndroid=false; isMacOS=false; isFedora=false; isBazzite=false isDebian=false; isArchLinux=false; isOpenSUSE=false; isAlpine=false
...
[ $isBazzite == true ] && scripts=(Bazzite adbInstall)
...
scripts+=(preferences art symbol menu confirmPrompt ghAuth fetchAssets dlGitHub fetchAppsInfo portSelection resetSelection viewPatches workflow APKMdl fileSelector managePatches editOptions editOptionsJson buildPatchCmd importExportSelection patchedApps dlPatchedApps Utils)
run() {
...
checkInternet &>/dev/null && { dlgh "REAndroid/APKEditor" "false" ".jar" "$simplifyNext" && APKEditorPath="$assetsPath" && unset "$assetsPath"; } || APKEditorPath=$(find "$simplifyNext" -type f -name "APKEditor-*.jar" -print -quit)
...
if [ $isAndroid == true ] && { [ $su == true ] || rish -c "id" &>/dev/null || adb -s $(adb devices 2>/dev/null | grep "device$" | awk '{print $1}' | tail -1) shell "id" &>/dev/null; }; then
...
EOF
      ...
      ([ "$cli" == "MorpheApp/morphe-cli" ] && [ $isAndroid == false ]) && sourceOptions+=("Launch GUI")
      [ $isAndroid == false ] && sourceOptions+=("Build Patches")
