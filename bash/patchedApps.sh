...
patchedAppsJson() {
  if jq -e --arg pkg "$package" '.. | (.packageName? // .name? // empty) | select(. == $pkg)' $sourceDir/patches-*.json >/dev/null; then
    if [ $ShowUniversalPatches == true ]; then
      appliedPatchesJson=$(jq --arg pkg "$package" '[.[] | 
        select(
          (.use == true or .default == true) and 
          (.compatiblePackages == null or any(.compatiblePackages[]?; 
            .name == $pkg or 
            ((.versions | type == "object") and .versions.packageName == $pkg)
          ))
        ) | {
          name: .name,
...
viewPatchedApps() {
  ...
  selected_patched_app=0
  while true; do
    menu names bButtons ppackages versions "$selected_patched_app" && selected_patched_app=$selected || break
    spackage="${spackages[selected_patched_app]}"
    ppackage="${ppackages[selected_patched_app]}"
    source="${sources[selected_patched_app]}"
    name="${names[selected_patched_app]}"
    version="${versions[selected_patched_app]}"
    patch="${patchs[selected_patched_app]}"
    patchedDate="${patchedDates[selected_patched_app]}"
    [ $isMacOS == true ] && patchedDateH=$(date -r $patchedDate +"%a %b %-d %Y %H:%M:%S") || patchedDateH=$(date -d @$patchedDate +"%a %b %-d %Y %H:%M:%S")
    patchedAge="${patchedAges[selected_patched_app]}"
    stock="${stocks[selected_patched_app]}"
    patched="${patcheds[selected_patched_app]}"
    installed="${installeds[selected_patched_app]}"
    mount="${mounts[selected_patched_app]}"
    appliedPatchesJson=$(jq -r ".[$selected_patched_app].patchUse" $simplifyNext/patchedApps.json)
    while true; do
      opts=("App Info" Action)
      ...
      case "$opt" in
        "App Info")
          ...
          if [ -f "$patched" ]; then
            [ $isAndroid == true ] && AppSize=$(awk "BEGIN {printf \"%.2f MB\n\", $(stat -c%s $patched 2>/dev/null)/1000000}" 2>/dev/null) || AppSize=$(du -sk "$patched" | awk '{total_bytes = $1 * 1024; printf "%.2f MB\n", total_bytes / 1000000}')
            echo "AppSize: $AppSize"
          fi
          echo; read -p "Press Enter to continue..."
          ;;
        Action)
          echo "Please Wait!!"
          actions=("Applied Patches" "Repatch")
          if { [ $isAndroid == false ] && [ -n "$serial" ]; } || { [ $isAndroid == true ] && { [ $su == true ] || rish -c "id" &>/dev/null || adb -s $(adb devices 2>/dev/null | grep "device$" | awk '{print $1}' | tail -1) shell "id" &>/dev/null; }; }; then
          ...
          if [ $installed == true ] || [ $mount == true ]; then
            { [ $isAndroid == false ] && [ -n "$serial" ]; } || { [ $isAndroid == true ] && ( [ $su == true ] || rish -c "id" &>/dev/null || adb -s $(adb devices | grep "device$" | awk '{print $1}' | tail -1) shell "id" &>/dev/null ); } && actions+=("Capture Patched App Runtime Logs")
          fi
          selected_action=0
          while true; do
            menu actions bButtons "" "" "$selected_action" && selected_action=$selected || break
            action="${actions[selected_action]}"
            case "$action" in
              Share) termux-open --send "$patched" ;;
              Open)
                [ $mount == true ] && package="$spackage" || package="$ppackage"
                shellCmd "monkey -p $package -c android.intent.category.LAUNCHER 1" &>/dev/null
                ;;
              Repatch)
                ...
                package="$spackage"; appName="$name"; link=$(jq --arg pkg "$package" -r '.[] | select(.package == $pkg) | .link' $sourceDir/apps.json)
                patchingWorkflow
                ;;
              Install|Reinstall)
                installed=true
              ...
              Mount|Remount)
                ...
                if [ $isAndroid == true ]; then
                  ...
                  adb -s "$serial" shell "[ ! -f '/data/local/tmp/apkMount.sh' ]" && { adb -s "$serial" push "$simplifyNext/apkMount.sh" /data/local/tmp/apkMount.sh &>/dev/null && adb -s "$serial" shell "chmod +x /data/local/tmp/apkMount.sh"; }
              ...
              Uninstall)
                ...
                if [ $isAndroid == false ]; then
                  shellCmd "pm uninstall $ppackage" &>/dev/null
                else
                  if [ $su == true ] || rish -c "id" &>/dev/null || adb -s $(adb devices 2>/dev/null | grep "device$" | awk '{print $1}' | tail -1) shell "id" &>/dev/null; then
                    shellCmd "pm uninstall $package" &>/dev/null
                  else
                    am start -a android.intent.action.UNINSTALL_PACKAGE -d package:"$ppackage" &>/dev/null
                    sleep 6
                    am start -a android.settings.APPLICATION_DETAILS_SETTINGS -d package:"$ppackage" &>/dev/null
                  fi
                fi
                ;;
              Unmount)
              ...
              Delete)
                ...
                ;;
              "Applied Patches") viewAppliedPatches ;;
              "Capture Patched App Runtime Logs")
                [ $mount == true ] && package="$spackage" || package="$ppackage"
                cmdOut=$(shellCmd "pm resolve-activity --brief $package")
                Activity=$(tail -1 <<< "$cmdOut")
                runCmd "logcat -c" # Clear Old Logs
                shellCmd "am start -n $Activity" &>/dev/null
                sleep 0.5
                PID=$(shellCmd "pidof $package")
                (runCmd "logcat --pid=$PID" > $SimplUsr/PatchedAppRuntimeLogs.txt 2>/dev/null & echo $! > $simplifyNext/pid)
                echo; read -p "Press Enter to save captured logs to $SimplUsr/PatchedAppRuntimeLogs.txt"
                shellCmd "killall logcat"
