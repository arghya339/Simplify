...
managePatches() {
  ...
  mapfile -t patchDescriptions < <(jq -r --arg pkg "$package" '.[] | select(any(.compatiblePackages[]?; if .name? and (.name|type)=="string" then .name == $pkg else .versions | if type=="object" then .packageName == $pkg elif type=="array" then any(.[].packageName? == $pkg) else false end end)) | (.description | tostring | gsub("\n"; " "))' $sourceDir/patches-$patchesVersion.json)
