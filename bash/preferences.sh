...
reloadConfig() {
  ...
  ContinueOnError=$(jq -r '.ContinueOnError' "$simplifyNextJson" 2>/dev/null)
  ...
  jq -e '.JvmHeapSize != null' "$simplifyNextJson" &>/dev/null && JvmHeapSize=$(jq -r '.JvmHeapSize' "$simplifyNextJson" 2>/dev/null) || JvmHeapSize=
}; reloadConfig
...
if [ $isAndroid == false ] && [ -n "$serial" ]; then
  config "ABI" "$cpuAbi"
...
if [ $isAndroid == false ]; then
  ...
  if jq -e '.LOCALE != null' "$simplifyNextJson" &>/dev/null; then
    locale="$(jq -r '.LOCALE' "$simplifyNextJson" 2>/dev/null)"
    lang=$(cut -d'-' -f1 <<< "$locale")
  else
    locale=; lang=
  fi
...
systemInfo() {
  ...
  echo "Java MaxHeapSize: $($java -XX:+PrintFlagsFinal -version 2>&1 | grep "MaxHeapSize" | awk '{print $4 / 1024 / 1024}' | uniq)M"
}
  ...
  while true; do
    ...
    case "$configureOption" in
      ...
      Updates)
        ...
        ;;
      Delete)
        deleteOptions=("Delete Stock Apk" "Delete Patched Apk" "Delete All Patched Log" "Delete All Mount Log" "Delete Patched App Runtime Logs" "Delete CLI" "Delete Patches" "Delete Integrations" "Delete MicroG" "Delete Keystore" "Delete Stock Apks After Successful Patching" "Delete Patched Apk After Installation" "Uninstall SimplifyNext")
        selected_del_opt=0
        while true; do
          ...
          case "$deleteOption" in
            ...
            "Delete Patched Apk")
              ...
              ;;
            "Delete All Patched Log") find "$SimplUsr" -maxdepth 1 -type f -name "*-*_v*-*.txt" -exec echo "Deleting: {}" \; -exec rm -f {} \; 2>/dev/null ;;
            "Delete All Mount Log") rm -f "$SimplUsr/mountLog.txt" "$SimplUsr/MountLog.txt" ;;
            "Delete Patched App Runtime Logs") rm -f "$SimplUsr/PatchedAppRuntimeLogs.txt" ;;
            "Delete CLI")
              while true; do
                    ...
                    YES)
                      ...
                      case "$response" in
                        Yes)
                          ...
                          [ $isAlpine == true ] && { pkgUninstall "libarchive-tools"; pkgUninstall "gcompat"; pkgUninstall "util-linux"; }
                          [ $isBazzite == false ] && pkgUninstall "glow"
                          ([ $isMacOS == true ] || [ $isBazzite == true ]) && { formulaeUninstall "android-platform-tools"; formulaeUninstall "android-commandlinetools"; }
                          #/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/uninstall.sh)"
                          ([ $isFedora == true ] || [ $isArchLinux == true ] || [ $isOpenSUSE == true ] || [ $isAlpine == true ]) && pkgUninstall "android-tools"
                          ([ $isDebian == true ] || [ $isAlpine == true ]) && pkgUninstall "curl"
                          [ $isDebian == true ] && { aptRemove "adb"; aptRemove "aapt"; }
                          [ $isArchLinux == true ] && yay -R android-sdk-build-tools --noconfirm
                          ([ $isFedora == true ] || [ $isOpenSUSE == true ] || [ $isAlpine == true ]) && rm -rf "$HOME/Android/Sdk/cmdline-tools/latest"
                        ;;
                      esac
        ...
        while true; do
          ...
          case "$importOption" in
            "Import Patch Selection")
              ...
              if menu existsSources bButtons existsPatches; then
            ...
            "Import Keystore")
              ...
              ;;
            "Import sources.json") fileSelector "json" && cp "$filePath" "$simplifyNext/sources.json" ;;
            "Import Script Settings") fileSelector "json" && cp "$filePath" "$simplifyNext/simplifyNext.json" ;;
            "Restore Script") fileSelector "zip" && pv "$filePath" | bsdtar -xf - -C "$simplifyNext" ;;
          esac
        done
        ;;
      Export)
        ...
        while true; do
          ...
          case "$exportOption" in
            ...
            "Export Keystore")
              ...
              ;;
            "Export sources.json") cp "$simplifyNext/sources.json" "$Download/sources.json" ;;
            "Export Script Settings") cp "$simplifyNext/simplifyNext.json" "$Download/simplifyNext.json" ;;
            "Backup Script") bsdtar --format=zip -c -f - -C "$simplifyNext" . | pv -t -b -r > "$Download/SimplifyNextBack.zip" ;;
          esac
        done
        ;;
      Reset)
        ...
        while true; do
          ...
          case "$resetOption" in
            ...
            "Reset Patch Options")
              ...
              ;;
            "Reset PatchSelection & PatchOptions")
              sources=($(jq -r '.[].source' $simplifyNext/sources.json))
              patches=($(jq -r '.[].patches' $simplifyNext/sources.json))
              if menu sources bButtons patches; then
                source="${sources[selected]}"
              ...
              ;;
            "Regenerate Keystore") genKeystore ;;
            "Reset Script Settings") rm -f "$simplifyNext/simplifyNext.json" && exit 0 ;;
          esac
        done
        ;;
      Advanced)
        advancedOptions=("Manage GitHub PAT" "Installation Options" "Show Universal Patches" "Enable SimplifyNext Optional Features" "Search engine" RipLib RipDpi RipLocale BytecodeMode ContinueOnError "Change Java Version" "Java Memory Limits")
        advancedDescriptions=("increases gh api rate limit" "Install APKs on-device with advanced options" "Enable or disable universal patches across all applications" "Enable / Disable SimplifyNext Optional Features" "Manage search engines for app version search" "Remove unused native libraries from patched apps" "Remove unused native dpi from stock split apks" "Remove unused native locale from stock split apks" "Set bytecode mode for Morphe CLI" "Continue patching even if a patch fails" "" "")
        ...
        while true; do
          ...
          case "$advancedOption" in
            ...
            BytecodeMode)
              ...
              ;;
            ContinueOnError)
              confirmPrompt "ContinueOnError" "tfButtons" "$ContinueOnError" && ContinueOnError=true || ContinueOnError=false
            ...
            "Show Universal Patches")
              ...
              ;;
            "Enable SimplifyNext Optional Features")
              confirmPrompt "Enable SimplifyNext Optional Features" "tfButtons" "$EnableOptionalFeatures" && EnableOptionalFeatures=true || EnableOptionalFeatures=false
              config "EnableOptionalFeatures" "$EnableOptionalFeatures"
              if [ $EnableOptionalFeatures == true ]; then
                if [ $isAndroid == true ]; then
                  installTermuxAPI
                elif [ $isMacOS == false ]; then
                  pkgInstall "mpv"
                  wl-copy -v &>/dev/null || aptInstall "wl-clipboard"  # Not Pre-installed on Debian, OpenSUSE, Alpine
                fi
              fi
              ;;
            "Search engine")
            ...
            "Java Memory Limits")
              ...
              if menu memOpts bButtons memDesc "" "$selected_mem_lim"; then
                ...
                case "$memOpt" in
                  Automatic)
                    [ -n "$JvmHeapSize" ] && jq 'del(.JvmHeapSize)' "$simplifyNextJson" > temp.json && mv temp.json "$simplifyNextJson"
                    JvmHeapSize=
                    ;;
                  Recommended)
                    JvmHeapSize="512"
                  ...
                  Maximum)
                    ...
                    ;;
                  Average)
                    JvmHeapSize="$AvgHeapSize"
                    config "JvmHeapSize" "$JvmHeapSize"
                    ;;
                  Manually)
                    read -r -p "JvmHeapSize(MB): " -i "512" -e ManHeapSize
                    [ -n "$ManHeapSize" ] && { JvmHeapSize="$ManHeapSize"; config "JvmHeapSize" "$JvmHeapSize"; }
                    ;;
                esac
              fi
              ;;
            "Change Java Version")
              if checkInternet; then
                ...
                while true; do
                  ...
                  elif [ $isMacOS == true ] || [ $isBazzite == true ]; then
