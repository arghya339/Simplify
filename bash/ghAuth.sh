...
ghPAT() {
  ...
  url="https://github.com/settings/tokens/new?scopes=public_repo,read:packages&description=simplifyNext" # Create a PAT with scope `public_repo`
  ...
  login=$(curl -sL -H "Authorization: Bearer $input" "https://api.github.com/user" | jq -r '.login')
  config "USER" "$login"
  config "PAT" "$input"
}
...
ghAuth() {
      ...
      case "$userInput" in
        Yes)
          if gh auth status >/dev/null 2>&1; then
            ...
            jq 'del(.USER)' "$simplifyNextJson" > temp.json && mv temp.json "$simplifyNextJson"
  ...
  if gh auth status >/dev/null 2>&1; then
    #ghUser="$(gh api /user --jq '.login' | cat)"
    ghUser="$(gh auth status 2>&1 | grep "account" | head -1 | awk '{print $7}')"
    ghToken="$(gh auth token)"
  elif jq -e '.PAT' "$simplifyNextJson" >/dev/null 2>&1; then
    ghUser="$(jq -r '.USER' "$simplifyNextJson" 2>/dev/null)"
