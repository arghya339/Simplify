...
def download_apk(base_url, package_name, versions, arch=None, download_dir="/storage/emulated/0/Download"):
    """
...
if __name__ == "__main__":
    # --- Determine the actual $HOME path ---
    ...
    if len(sys.argv) > 1:
        ...
        if versions:
            downloaded_apk = download_apk(base_download_url, package_name, versions)
            if downloaded_apk:
    ...
    # --- If no argument is provided, download YouTube, YouTube Music, and Reddit ---
        ...
        if youtube_versions:
            downloaded_youtube_apk = download_apk(
                base_download_url, youtube_package_name, youtube_versions
            )
        ...
        arch = subprocess.check_output(["getprop", "ro.product.cpu.abi"]).decode().strip()
        ...
        reddit_versions = ["2025.05.1-2214840"]  # Replace with the actual versions

        if reddit_versions:
            downloaded_reddit_apk = download_apk(
                base_download_url, reddit_package_name, reddit_versions
            )
