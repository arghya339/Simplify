        ...
        if [ "$pkgName" == "com.google.android.youtube" ]; then
          ...
          case $opt in
            Install)
              ...
              termux-wake-lock
              cs "${stock_apk_ref[0]}" "$outputAPK" "$SimplUsr/${appNameRef[0]}-Morphe-CS_v${pkgVersion}-$Arch.apk"
              termux-wake-unlock
              echo -e "$running Please Wait !! Installing Patched ${appNameRef[0]} Morphe CS apk.."
              apkInstall "$SimplUsr/${appNameRef[0]}-Morphe-CS_v${pkgVersion}-$Arch.apk" ""
              ;;
            Mount)
              ...
              rm $outputAPK
              ;;
            Cancel) echo -e "$notice ${appNameRef[0]} Morphe Installaion skipped!" ;;
          esac
        else
...
while true; do
  ...
  case "$selected" in
    ...
    YouTube)
      ...
      [ $Android -eq 9 ] && pkgVersion="21.23.492" || pkgVersion=""
      ([ $Android -ge 10 ] && [ $FetchPreRelease -eq 1 ]) && pkgVersion="21.26.360"
    ...
    YTMusic)
      ...
      pkgVersion=""
      [ $FetchPreRelease -eq 1 ] && pkgVersion="9.33.52"
      if [ -z "$pkgVersion" ]; then
        getVersion "$pkgName"
        pkgVersion="$pkgVersion"
      fi
    ...
    Reddit)
      ...
      ([ $Android -gt 9 ] && [ $FetchPreRelease -eq 1 ]) && pkgVersion="2026.35.0"
      if [ -z "$pkgVersion" ]; then
        getVersion "$pkgName"
        pkgVersion="$pkgVersion"
      fi
      ...
      build_app "$pkgName" "appName" "$pkgVersion" "$Type" "Arch" "reddit_patches_args" "$pkgName" "$activityPatched"
      ;;
  esac
