...
getListOfPatches() {
  ...
  Patches=$(curl -sL 'https://raw.githubusercontent.com/LisoUseInAIKyrios/revanced-patches/refs/heads/dev/bundles/lisouseInaikyrios-latest-patches-list.json' | jq --arg pkgName "$pkgName" '.patches[] | select(.compatiblePackages != null and .compatiblePackages[$pkgName] != null)')
  if [ "$ReadPatchesFile" -eq 1 ]; then
