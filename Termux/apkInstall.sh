...
apkInstall() {
  ...
  if su -c "id" >/dev/null 2>&1; then
    ...
    [ $DisablePlayProtect -eq 1 ] && "$HOME/adb" -s $(~/adb devices | grep "emulator-*" | awk '{print $1}') shell "settings put global package_verifier_user_consent -1"  # Disabled Play Protect
    output=$(~/adb -s $(~/adb devices | grep "emulator-*" | cut -f1) shell pm install ${cmd} "$outputAPK" 2>&1)
    #$HOME/adb -s $(~/adb devices | grep "emulator-*" | awk '{print $1}') install ${cmd} "$outputAPK" 2>&1
    #~/adb -s $(~/adb devices | grep "emulator-*" | awk '{print $1}') shell cmd package install ${cmd} "$outputAPK" > /dev/null 2>&1
    [ $DisablePlayProtect -eq 1 ] && ~/adb -s $(~/adb devices | grep "emulator-*" | awk '{print $1}') shell "settings put global package_verifier_user_consent 1"  # Enabled Play Protect
    if [[ $output == *"Downgrade detected"* ]] && [ $KeepsData -eq 1 ]; then
      echo -e "${Green}$appName uninstall successfully with keeps app data.${Reset}\n${Yellow}Don't forget to restart Simplify after reboot!${Reset}"
      ~/adb -s $(~/adb devices | grep "emulator-*" | cut -f1) shell "cmd package uninstall -k $pkgName"
      cp "$outputAPK" "$POST_INSTALL"
      sleep 12
      "$HOME/adb" -s $(~/adb devices | grep "emulator-*" | awk '{print $1}') "reboot"
    fi
    am start -n "$activityClass" &> /dev/null  # launch app after update
    ...
    if [ $EnableRoolback -eq 1 ]; then
      ...
      if [[ "$response" == [Yy]* ]]; then
        ...
        if su -c "id" >/dev/null 2>&1; then
          ...
          $HOME/adb -s $(~/adb devices | grep "emulator-*" | awk '{print $1}') shell "pm rollback-app $pkgName"
        fi
      fi
    fi
  fi
